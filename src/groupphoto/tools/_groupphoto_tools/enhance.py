...
log = logging.getLogger("groupphoto.enhance")

_NCNN_BIN = "realesrgan-ncnn-vulkan"
_WEIGHTS_DIR = os.path.expanduser(
    os.environ.get("FW_GROUPPHOTO_WEIGHTS_DIR", "~/.cache/groupphoto/weights"))
...
def _torch_device() -> str:
    """MPS (Apple GPU) → CUDA → CPU, overridable with FW_GROUPPHOTO_DEVICE."""
    dev = os.environ.get("FW_GROUPPHOTO_DEVICE")
    if dev:
        return dev
    try:
...
def _upscale_realesrgan_py(img: Any, scale: int) -> Any | None:
    ...
    try:
        ...
        return None
    weights = _weights_path("RealESRGAN_x4plus.pth", "FW_GROUPPHOTO_REALESRGAN_WEIGHTS")
...
def _restore_gfpgan(img: Any, fidelity: float) -> Any | None:
    ...
    try:
        ...
        return None
    weights = _weights_path("GFPGANv1.4.pth", "FW_GROUPPHOTO_GFPGAN_WEIGHTS")
    if not os.path.isfile(weights):
...
def _face_device() -> str:
    ...
    gaps. Override with FW_GROUPPHOTO_FACE_DEVICE."""
    return os.environ.get("FW_GROUPPHOTO_FACE_DEVICE", "cpu")
...
def _restore_codeformer(img: Any, fidelity: float) -> Any | None:
    ...
    try:
        ...
        return None
    weights = _weights_path("codeformer.pth", "FW_GROUPPHOTO_CODEFORMER_WEIGHTS")
    if not os.path.isfile(weights):
