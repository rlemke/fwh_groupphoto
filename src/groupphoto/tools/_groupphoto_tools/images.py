...
log = logging.getLogger("groupphoto.images")
...
def load_image16(path: str | Path, *, highlight_mode: str = "clip") -> Any:
    ...
    ``highlight_mode`` (RAW only) controls LibRaw highlight handling — the strongest
    lever for blown windows/sky behind a group, since it reconstructs detail from any
    channel that stayed below clipping:
      ``clip`` (default) — hard clip (LibRaw mode 0);
      ``blend`` — gentle rolloff (mode 2);
      ``reconstruct`` — aggressive reconstruction (mode 5).
    No effect on non-RAW (already-clipped) inputs.
    """
    import numpy as np  # noqa: PLC0415
    ...
    if p.suffix.lower() in RAW_EXTS:
        ...
        # Recovered highlights need headroom not to re-clip → disable auto-bright
        # when actively recovering (blend/reconstruct); keep it for plain clip.
        kw = dict(use_camera_wb=True, output_bps=16, highlight_mode=hl,
                  no_auto_bright=(hl != 0))
